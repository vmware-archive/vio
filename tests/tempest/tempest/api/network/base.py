...
class BaseDvsAdminNetworkTest(BaseAdminNetworkTest):

    @classmethod
    def resource_cleanup(cls):
        for port in cls.ports:
            cls.admin_client.delete_port(port['id'])
        for subnet in cls.subnets:
            cls.admin_client.delete_subnet(subnet['id'])
        for network in cls.networks:
            cls.admin_client.delete_network(network['id'])
    ...
    def create_network(cls, network_name=None):
        ...
        if tempest.test.is_extension_enabled('provider', 'network'):
            ...
            body = cls.admin_client.create_network(**body)
        network = body['network']
        cls.networks.append(network)
        return network
    ...
    def create_subnet(cls, network):
        ...
        for subnet_cidr in cidr.subnet(mask_bits):
            try:
                body = cls.admin_client.create_subnet(
                    network_id=network['id'],
                    cidr=str(subnet_cidr),
    ...
    def create_port(cls, network, **kwargs):
        """Wrapper utility that returns a test port."""
        body = cls.admin_client.create_port(network_id=network['id'],
                                            **kwargs)
        port = body['port']
        cls.ports.append(port)
        return port
